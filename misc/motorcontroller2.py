...
class MotorController():
  ...
  def __init__(self):
    ...
    self.motor = Motor(self.pwm_a_pin, self.pwm_b_pin)
  ...
  def loop(self):
    ...
    if abs(error) < 2 and abs(speed) < 500:
      self.motor.stop()
      ...
      if control > 0:
        self.motor.forward(self.maximum_power if control > self.maximum_power else control)
        self.stability = 0
        # threading.Timer(self.interval, self.loop).start()
      else:
        self.motor.backward(self.maximum_power if -control > self.maximum_power else -control)
        self.stability = 0
        # threading.Timer(self.interval, self.loop).start()
